       ...
       if(!empty($request->comment_name)){
          ...
          $coment->isauth=0;
           ...
           $coment->user_id=Auth::user()->id;
            ...
            $coment->project_id=$request->pj_id;
 ...
 $output .= $this->get_reply_comment( $comment->id);
    ...
    */
    ...
    public function getComent(Request $request){
        ...
        $coments=CommentProject::where('project_id',$request->pj_id)->where('parent_id',0)->get();
 ...
 $output .= $this->get_reply_comment( $comment->id);
     ...
     function get_reply_comment( $parent_id = 0, $marginleft = 0){
        ...
        if($parent_id==0 ){
           ...
           $rep_comments=CommentProject::where('parent_id',$parent_id)->get();
          // dd($rep_comments);
    ...
    function showDate( $date ) // $date --> время в формате Unix time
{
 ...
 public function getCount(Request $request){
     $coment=CommentProject::where('project_id',$request->pj_id)->get();
     $count=count($coment);
