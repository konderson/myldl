...
class CommentDiaryController extends Controller
{
     public function addcomment(Request $request){
        
       $coment=new CommentDiary();
       if(!empty($request->comment_name)){
          $coment->author_name=$request->comment_name;
          $coment->isauth=0;
       }
       else{
            $coment->author_name=Auth::user()->name;
           $coment->user_id=Auth::user()->id;
            $coment->isauth=1;
       }
    
            $coment->text=$request->comment_content;
            $coment->diary_id=$request->diary_id;
 ...
 $output .= $this->get_reply_comment( $comment->id);
    ...
    */
    ...
    public function getComent(Request $request){
        ...
        $coments=CommentDiary::where('diary_id',$request->diary_id)->where('parent_id',0)->get();
 ...
 $output .= $this->get_reply_comment( $comment->id);
     ...
     function get_reply_comment( $parent_id = 0, $marginleft = 0){
        ...
        if($parent_id==0 ){
           ...
           $rep_comments=CommentDiary::where('parent_id',$parent_id)->get();
          // dd($rep_comments);
    ...
    function showDate( $date ) // $date --> время в формате Unix time
{
 ...
 public function getCount(Request $request){
     $coment=CommentDiary::where('diary_id',$request->diary_id)->get();
     $count=count($coment);
