...
@section('content')
    ...
    <ol class="breadcrumb">
        <li><a href="/">Главная</a></li><li class="active">Новости</li>    </ol>
...
<script>
     (adsbygoogle = window.adsbygoogle || []).push({});
                            ...
                            <div class="date"><span>{{ Carbon\Carbon::parse($fn->created_at)->format('d.m.Y') }}</span></div>
                                <a href="/news/item/{{$fn->id}}"  style="margin-right: 15px;" class="advert-row-body-title">{{$fn->name}}</a>
                            <div class="advert-row-body news-row-body">
                                <p class="adv-info">{!!  strip_tags (substr($fn->text, 0, 150))!!}....</p>
                            </div>
                        </div>
					                          @endforeach
					       ...
					       @foreach($news as $ns)
                            ...
                            <div class="advert-row-body news-row-body">
                                <p class="adv-info">{!!  strip_tags (substr($ns->text, 0, 150))!!}....</p>
                            </div>
                        </div>
					                          @endforeach
