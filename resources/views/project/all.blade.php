...
@section('content')
    ...
    <ol class="breadcrumb">
        <li><a href="/">Главная</a></li><li class="active">
Наши проекты</li>    </ol>
        ...
        <div class="left">
                        ...
                        <input type="submit" class="search-magnify" value="">
					       ...
					       @foreach($projects as $pj)
                            ...
                            <div class="date"><span>{{ Carbon\Carbon::parse($pj->created_at)->format('d.m.Y') }}</span></div>
                                <a href="/project/item/{{$pj->id}}"  style="margin-right: 15px;" class="advert-row-body-title">{{$pj->name}}</a>
                                <div class="image">
			<img src="{{asset('storage/help/'.$pj->image)}}"/>
	                        	</div>
                            <div class="advert-row-body news-row-body">
                                <p class="adv-info">{!!  strip_tags (substr($pj->text, 0, 150))!!}....</p>
                            </div>
                        </div>
					                          @endforeach
            ...
            <div class="right">
                ...
                <div class="advert-body">
                    ...
                    @foreach($lprojects as $lpj)
					                        <div class="article-info">
                            <div class="article-subtitle">
                                <div class="date"><span>{{ Carbon\Carbon::parse($lpj->created_at)->format('d.m.Y') }}</span></div>
                                <a href="/project/item/{{$lpj->id}}">{{$lpj->name}}</a>
                            </div>
                        </div>
					 @endforeach             
                        </div>
