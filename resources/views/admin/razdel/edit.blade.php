...
<div id="main-content"> <!-- Main Content Section with everything -->
			...
			<!-- Page Head -->
			<h2>Редактирование записи - {{$razdel->name}}</h2>
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-content">
							...
							<form class="right" action="{{route('admin.adminpanel.razdel.update')}}" method="post">
                                  @csrf
                                   @method('PUT')
							...
							<fieldset> <!-- Set class to "column-left" or "column-right" on fieldsets to divide the form into columns -->
								...
								<p>
									<input class="text-input small-input" type="text" name="name" value="{{$razdel->name}}" />
								...
								<select class="selected" name="flag" >
									<option value="1" <?if($razdel->flag==1) echo("SELECTED")?>>Открытый</option>
									<option value="0" <?if($razdel->flag==0) echo("SELECTED")?> >Закрытый</option>
								</select>
								<span class="input-notification information">Флаг</span>&nbsp;&nbsp;
							</p>
									
                                                    
                            	<input type="hidden" name="id" value="{{$razdel->id}}" />
